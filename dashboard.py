...
def detect_config(args=None):
    ...
    # Normalize args into a mutable namespace so detect_config(None) works for
    # tooling/introspection flows in addition to CLI bootstraps.
    if args is None:
        args = argparse.Namespace()
    for field in ('workspace', 'sessions_dir', 'log_dir', 'name', 'data_dir'):
    ...
    if data_dir and os.path.isdir(data_dir):
        ...
        if os.path.isdir(ws) and not args.workspace:
            args.workspace = ws
        sess = os.path.join(data_dir, 'agents', 'main', 'sessions')
        if os.path.isdir(sess) and not args.sessions_dir:
    ...
    # Route registration is intentionally centralized and idempotent to keep
    # startup deterministic across CLI runs, tests, and tooling probes.
    try:
        _register_blueprints_once()
    except Exception:
        pass
...
def _load_gw_config():
    ...
    # Environment token takes precedence over persisted config so CI/test runs
    # can override local machine state deterministically.
    env_token = os.environ.get('OPENCLAW_GATEWAY_TOKEN', '').strip()
    env_url = os.environ.get('OPENCLAW_GATEWAY_URL', '').strip()
    if env_token:
        GATEWAY_TOKEN = env_token
        GATEWAY_URL = env_url or f'http://127.0.0.1:{_detect_gateway_port()}'
        return {'url': GATEWAY_URL, 'token': GATEWAY_TOKEN}
    # Try config file
    try:
