...
class TursoDataProvider(ClawMetryDataProvider):
    ...
    def _query(self, sql, params=None):
        ...
        req = urllib.request.Request(
            self.turso_url.rstrip("/") + "/v2/pipeline", data=payload,
        ...
        try:
            ...
            return [{cols[i]: (cell.get("value") if cell.get("type") != "null" else None)
    ...
    def list_sessions(self, limit=30, include_subagents=True, since_ms=None):
        ...
        if self.node_id:
            sql += " AND node_id=?"; params.append(self.node_id)
        sql += " ORDER BY updated_at DESC LIMIT ?"; params.append(limit)
        return [Session(session_id=r.get("session_id",""), display_name=r.get("display_name",""),
            model=r.get("model",""), channel="", updated_at=int(r.get("updated_at") or 0),
            total_tokens=int(r.get("total_tokens") or 0)) for r in self._query(sql, params)]

    def get_session(self, session_id):
        rows = self._query("SELECT * FROM sessions WHERE session_id=? AND owner_hash=? LIMIT 1",
    ...
    def get_session_index(self):
        return {s.session_id: {"session_id": s.session_id, "display_name": s.display_name,
            "model": s.model, "updated_at": s.updated_at, "total_tokens": s.total_tokens}
            for s in self.list_sessions(limit=200)}

    def get_events(self, session_id, limit=500, tail_bytes=None):
        rows = self._query("SELECT * FROM events WHERE owner_hash=? AND session_id=? ORDER BY ts DESC LIMIT ?",
                           [self.owner_hash, session_id, limit])
        return [Event(event_id=str(i), session_id=r.get("session_id",""),
            event_type=r.get("event_type",""), ts=r.get("ts",""),
            data=json.loads(r.get("data") or "{}")) for i, r in enumerate(rows)]

    def get_log_lines(self, date_str=None, limit=1000): return []
    def list_log_dates(self, days_back=31): return []
    def list_memory_files(self): return []
    def read_workspace_file(self, relative_path): return ""
    def list_crons(self): return []
